...
interface QuestionaireStep3Props {
  formData: QuestionaireInterface
  setFormData: (formData: QuestionaireInterface) => void
}

const QuestionaireStep3 = ({
  formData,
  setFormData,
}: QuestionaireStep3Props) => {
    ...
    <div className="flex h-fit w-full flex-col gap-4">
      ...
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-base font-bold">
          <span>{t('routes.questionaireGroup.part3.header')}</span>:{' '}
          <span>{t('routes.questionaireGroup.part3.title')}</span>
        </h2>
      </div>
        ...
        <>
          <div key={key} className="flex flex-col gap-2">
            <p className="font-semibold">
              {index + 1}. {t(`routes.questionaireGroup.part3.${key}.question`)}{' '}
              <RedStar />
            </p>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-black">
                {t('routes.questionaireGroup.part3.rating.low')}
              </span>

              {/* Rating Icons */}
              <div className="flex w-full justify-between px-4">
                {RATING_VALUES.map((r) => {
                  const isActive = formData.part3[key] === r
              ...
              {/* High */}
              <span className="text-sm text-black">
                {t('routes.questionaireGroup.part3.rating.high')}
              </span>
            </div>
          </div>
          <BreakLine />
      ...
      <div className="flex flex-col gap-2">
        <p className="font-semibold">
          5. {t('routes.questionaireGroup.part3.q5.question')}
        </p>
        <Input
          placeholder={t('routes.questionaireGroup.part3.q5.placeholder')}
          className="border-main-pink text-main-pink placeholder:text-main-light-pink"
          value={formData.part3.q5}
          onChange={(e) =>
            setFormData({
              ...formData,
              part3: {
...
export default QuestionaireStep3
