  ...
  const faqs = [
    {
      ...
      answer:
        'งานของเราจะจัดขึ้นในวันที่ 28 - 29 มีนาคม 2026 เวลา 08:00 - 16:00 น.',
    },
    {
      index: '1',
      ...
      >
        {faqs.map((faq) => (
          <AccordionItem value={faq.index} key={faq.index} className="border-0">
            <AccordionTrigger
              className={clsx(
                'rounded-t-[10px] bg-white px-4 py-2 text-sm leading-5 font-normal transition-all duration-300',
            ...
            <AccordionContent
              className={clsx(
                'bg-main-beige by-1 rounded-b-[10px] px-4! py-2! text-xs leading-4.5 font-normal',
                'overflow-hidden whitespace-pre-wrap transition-all duration-300 ease-in-out',
