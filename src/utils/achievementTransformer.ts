...
function getFacultyName(value: FacultyType, isEnglish: boolean) {
  ...
  const name = isEnglish ? faculty.label.en : faculty.label.th
...
function getAbbrFacultyName(value: FacultyType, isEnglish: boolean) {
  ...
  const name = isEnglish ? faculty.abbrLabel.en : faculty.abbrLabel.th
...
export function transformAchievement(
  ...
  if (achievement.variant === 'var3') {
    return {
      ...achievement,
      faculty: getFacultyName(achievement.faculty as FacultyType, isEnglish),
    }
  }
